...
window.addEventListener('load', () => {
    new Vue({
        ...
        methods: {
            ...
            MAxMinCourse(){
                console.log("Проверка метода MAxMinCourse");
            }
        },
        computed: {
            myNout() {
                ...
                this.MAxMinCourse();
                return {
                    // Здесь надо указать переменную у которой значение true или false
                    min: this.mynotebook,
                    /*min: this.selectedCurses
                        .some((item) => item.course === val.course.prices['maximum'] && item.tax === price),*/
        ...
        watch: {
            mynotebook() {
                ...
                // this.discountStyle = this.selectedCurses.every(item => console.log('Отработал метод mynotebook ', item.course.prices['maximum'], item.course.prices['medium']));
                this.discountStyle = this.selectedCurses.every(()=> true);
                // console.log("Это changePriceNew ", this.changePriceNew);
                console.log("Это discountStyle", this.discountStyle);
            }
        },
        filters: {
            PromoCodSend(value) {
